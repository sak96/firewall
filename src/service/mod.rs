...
impl AppWall {
    ...
    async fn run_loop(self) -> std::io::Result<()> {
        let (ctrl_c_handler, mut terminate) = tokio::sync::oneshot::channel();
        tokio::spawn(async move {
        ...
        while terminate.try_recv().is_err() {
            tokio::select! {
                ...
                Ok(mut msg) = queue.recv() => {
                    ...
                    tokio::spawn(async move {
                        ...
                        match TrafficPacket::from(payload) {
                            ...
                            Ok(mut pkt) => {
                                verdict =  if pkt.pid == std::process::id() {
                                    info!("ignore self packet");
                                    Verdict::Accept
                                } else if !pkt.dns_data.is_empty() {
